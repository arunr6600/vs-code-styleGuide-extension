...
class MyWebView {
    ...
    _disposables = [];
    _cachedResources;
    static createOrShow(extensionUri, label, key, cachedResources) {
        ...
        const panel = vscode.window.createWebviewPanel('myWebView', 'My Web View', column, {
            enableScripts: true,
    ...
    constructor(panel, extensionUri, label, key, cachedResources) {
        this._panel = panel;
    ...
    async _update(label, key) {
        const webview = this._panel.webview;
        const { preview, syntax } = (0, extension_1.getCachedContent)(key);
        this._panel.title = label;
        this._panel.webview.html = await this._getHtmlForWebview(webview, label, preview, syntax);
    }
    async _getHtmlForWebview(webview, label, previewHtml, syntaxHtml) {
